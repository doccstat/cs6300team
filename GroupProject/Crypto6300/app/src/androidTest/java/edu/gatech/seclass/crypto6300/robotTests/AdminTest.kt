...
class AdminTest {
    ...
    @Before
    fun setup() {
        AppDatabase.getInstance(activityTestRule.activity)?.clearAllTables()
        AppDatabase.getInstance(activityTestRule.activity)?.userDao()?.insert(adminUser)
    ...
    @Test
    fun addCryptogram_WillFailWithCryptogramExists() {
        loginAsAdminCorrectly()
        navToAddCryptogram()
        addCryptogramCorrectly()
        navToAddCryptogram()
    ...
    private fun addCryptogramCorrectly() {
        admin {
            setCryptogramName("aristotle")
            setCryptogramSolution("android")
            setNumEasyAttempts("7")
